...
def log_log_plot(file1, file2, sample, vec_x, vec_y, first):
    ...
    rel_ab = pd.to_numeric(merged_df['Mean']) / np.sum(pd.to_numeric(merged_df['Mean']))*100;
    rel_ab_sylph = pd.to_numeric(merged_df['Sequence_abundance']) / np.sum(pd.to_numeric(merged_df['Sequence_abundance']))*100;

    rel_ab_H = pd.to_numeric(merged_df_H['Mean']) / np.sum(pd.to_numeric(merged_df_H['Mean']))*100;
    rel_ab_sylph_H = pd.to_numeric(merged_df_H['Sequence_abundance']) / np.sum(pd.to_numeric(merged_df_H['Sequence_abundance']))*100;

    rel_ab_L = pd.to_numeric(merged_df_L['Mean']) / np.sum(pd.to_numeric(merged_df_L['Mean']))*100;
    rel_ab_sylph_L = pd.to_numeric(merged_df_L['Sequence_abundance']) / np.sum(pd.to_numeric(merged_df_L['Sequence_abundance']))*100;
    ...
    plt.ylabel('Sylph abundance (log scale)')
    plt.xlabel('Alignment abundance (log scale)')
...
plt.title("Read alignment versus sylph abundance on 50 real gut metagenomes")
plt.gca().spines['top'].set_visible(False)
plt.gca().spines['right'].set_visible(False)
plt.legend(frameon=False)
plt.savefig("supp_figs/read_align_50_gut.svg")
plt.show()
