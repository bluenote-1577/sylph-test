...
output_results_folder = "./synthetic_simulated_results_75gap/"
output_results_folder_95 = "./synthetic_simulated_results_95_75gap/"
output_results_folder_85 = "./synthetic_simulated_results_85_75gap/"
    ...
    run:
        for it in iters:
            for ab in abundances:
                for k in ks:
                    for c in cs:
                        shell(f"prita contain {input_genome} synthetic_simulated_reads/{it}-{ab}-synthetic.fq -c {c} -k {k} --ci -m 0.75 > {output_results_folder}/{k}-{c}-{it}-{ab}.tsv -t3")
    ...
    run:
        for it in iters:
            for ab in abundances:
                for k in ks:
                    for c in cs:
                        shell(f"prita contain {input_genome_95} synthetic_simulated_reads/{it}-{ab}-synthetic.fq -c {c} -k {k} --ci -m 0.75 > {output_results_folder_95}/{k}-{c}-{it}-{ab}.tsv -t3")
    ...
    run:
        for it in iters:
            for ab in abundances:
                for k in ks:
                    for c in cs:
                        shell(f"prita contain {input_genome_85} synthetic_simulated_reads/{it}-{ab}-synthetic.fq -c {c} -k {k} --ci -m 0.75 > {output_results_folder_85}/{k}-{c}-{it}-{ab}.tsv -t3")
